...
export interface ConnectionTrafficData {
  id: string
  domain: string
  ip: string
  uploadTotal: string
...
export function useDeviceTrafficQuery() {
  ...
  return useQuery({
    queryKey: [...QUERY_KEY_TRAFFIC, 'device'],
    queryFn: async () => {
      const data = await gqlClient.request<{ deviceTraffic: DeviceTrafficData[] }>(
        `
          query DeviceTraffic {
            deviceTraffic {
              ip
...
export function useConnectionTrafficQuery() {
  ...
  return useQuery({
    queryKey: [...QUERY_KEY_TRAFFIC, 'connection'],
    queryFn: async () => {
      const data = await gqlClient.request<{ connectionTraffic: ConnectionTrafficData[] }>(
        `
          query ConnectionTraffic {
            connectionTraffic {
              id
              domain
              ip
